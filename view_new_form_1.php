...
<div id="machine_hours_display">machine_hours</div>
</form>
    ...
    document.getElementById("project_group_number").addEventListener("change", function () {
        ...
        if (projects_buleg !== "") {
            ...
            .then(data => {
               ...
               const container = document.getElementById("machine_hours_display");
               container.innerHTML = ""; // Clear previous

        for (const [group, machines] of Object.entries(data)) {
        const groupTitle = document.createElement("h4");
        groupTitle.textContent = group;
        container.appendChild(groupTitle);

        for (const [machine, hours] of Object.entries(machines)) {

            const p = document.createElement("p");
            p.textContent = `${machine}: ${hours}`;
