...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["project"])) {
    ...
    $checklist = $_POST['checkbox_values'] ?? [];
    $checklist_values = array_values($checklist);
    $checklist_sql = '[' . implode(';', $checklist_values) . ']';
    $hereglegch_ner = $_POST["username"] ?? '';
    $ognoo = $_POST["ognoo"] ?? '';
    ...
    $sql = "INSERT INTO new_form (
        project, ded_buleg, ded_buleg_checklist, hereglegch_ner, ajliin_turul, ognoo,
    ...
    if ($stmt->execute()) {
        header("Location: new_form_css_test_1.php");
